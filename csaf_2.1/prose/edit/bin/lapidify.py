    ...
    for slot, line in enumerate(lines):
        ...
        for tag in sec_cnt:
            if line.startswith(tag) and clean_headings:
                ...
                if line.rstrip() == '# References':
                    line = line.rstrip().replace('# ', '# Appendix A. ', 1) + '{.unnumbered #references}' + NL


                if line.rstrip() == '## Normative References':
                    line = line.rstrip().replace('## ', '## A.1 ', 1) + '{.unnumbered #normative-references}' + NL

                if line.rstrip() == '## Informative References':
                    line = line.rstrip().replace('## ', '## A.2 ', 1) + '{.unnumbered #informative-references}' + NL


                if line.rstrip() == '# Safety, Security, and Data Protection Considerations':
                    line = line.rstrip().replace('# ', '# Appendix B. ', 1) + '{.unnumbered #safety-security-and-data-protection-considerations}' + NL


                if line.rstrip() == '# Acknowledgments':
                    line = line.rstrip().replace('# ', '# Appendix C. ', 1) + '{.unnumbered #acknowledgments}' + NL


                if line.rstrip() == '# Revision History':
                    line = line.rstrip().replace('# ', '# Appendix D. ', 1) + '{.unnumbered #revision-history}' + NL


                if line.rstrip() == '# Notices':
                    line = line.rstrip().replace('# ', '# Appendix E. ', 1) + '{.unnumbered #notices}' + NL


                if line.rstrip() == '# Guidance on the Size of CSAF Documents':
                    line = line.rstrip().replace('# ', '# Appendix F. ', 1) + '{.unnumbered #guidance-on-the-size-of-csaf-documents}' + NL


                if line.rstrip() == '## File Size':
                    line = line.rstrip().replace('## ', '## F.1 ', 1) + '{.unnumbered #file-size}' + NL

                if line.rstrip() == '## Array Length':
                    line = line.rstrip().replace('## ', '## F.2 ', 1) + '{.unnumbered #array-length}' + NL

                if line.rstrip() == '## String Length':
                    line = line.rstrip().replace('## ', '## F.3 ', 1) + '{.unnumbered #string-length}' + NL

                if line.rstrip() == '## URI Length':
                    line = line.rstrip().replace('## ', '## F.4 ', 1) + '{.unnumbered #uri-length}' + NL

                if line.rstrip() == '## Enum':
                    line = line.rstrip().replace('## ', '## F.5 ', 1) + '{.unnumbered #enum}' + NL

                if line.rstrip() == '## Date':
                    line = line.rstrip().replace('## ', '## F.6 ', 1) + '{.unnumbered #date}' + NL

                if 'safety-security-and-data-protection-considerations' == label:
                    print(xline)
