### Conformance Clause 2: CSAF Producer

A program satisfies the "CSAF Producer" conformance profile if the program:

- produces output in the CSAF format, according to the conformance profile "CSAF Document".
- satisfies those normative requirements in sections [sec](#schema-elements) and [sec](#safety-security-and-data-protection-considerations)
  that are designated as applying to CSAF Producers.
- satisfies those normative requirements in sections [sec](#extensions) and [sec](#safety-security-and-data-protection-considerations)
  that are designated as applying to CSAF Tools.
