...
/ ​Advisory Management System<def:advisory-management-system>: software
    ...
    organizations to assess the quality of these software artifacts at a
    point in time, and performs functions such as filing security
...
/ ​Artifact<def:artifact>: sequence of bytes addressable via a URI.
    ...
    specific version of a file in a version control system; a database
...
#strong[\[]<RFC4647>#strong[RFC4647\]] Phillips, A., Ed. and M. Davis,
Ed., "Matching of Language Tags", BCP 47, RFC 4647, DOI
10.17487/RFC4647, September 2006,
#link("https://www.rfc-editor.org/info/rfc4647/").
...
#link("https://www.rfc-editor.org/info/rfc5646/").
...
#strong[\[]<VERS>#strong[VERS\]] #emph[VErsion Range Specifier (VERS)
Specification], Part of the package-URL GitHub Project,
...
\[#link(<JSON-Schema-Core>)[JSON-Schema-Core]\]). To represent paths
which contain the branch recursion below `$.product_tree.branches[*]`
the descendant segment syntax `$.product_tree..branches[*]` is used.
Implementations need to ensure that only matching paths with the
intended type are selected. For example, this could be done by checking
that the property names that are omitted through the `..` syntax along
the path between `product_tree` and `branches` are just `branches`.
Including but not limited to paths containing `x_extensions` could
introduce arbitrary other property names.
...
The value `significant` indicates, that the content provided through
this extension is highly relevant and significantly aids in
understanding the overall content of the CSAF document. CSAF consumers
and CSAF validators SHOULD warn if they process a CSAF Document
including such an extension instance and do not have the extension in
question already implemented.

The value `supplementary` indicates, that the content provided through
this extension provides additional information. CSAF consumers and CSAF
validators MAY warn if they process a CSAF Document including such an
extension instance and do not have the extension in question already
implemented.
...
+ VErsion Range Specifier (VERS)
  ...
  VERS SHALL be used in its canonical form. To convey the term "all
  versions", the special string `vers:all/*` SHALL be used.
...
<full-product-name-type---product-identification-helper---hashes>
List of hashes (`hashes`) of value type `array` with `1` or more unique
items contains a list of cryptographic hashes usable to identify files.
...
List of file hashes (`file_hashes`) of value type `array` with `1` or
more unique items contains a list of cryptographic hashes for this file.
...
more unique items contains a list of URLs where SBOMs for this product
can be retrieved.
...
or more unique items contains a list of stock keeping units.
...
<full-product-name-type---product-identification-helper---generic-uris>
List of generic URIs (`x_generic_uris`) of value type `array` with `1`
or more unique items contains a list of identifiers which are either
...
#quote(block: true)[
The pattern given above is derived from the ABNF depicted in Figure 1 in
\[#link(<RFC5646>)[RFC5646]\]. CSAF skips those grandfathered language
tags that are deprecated at the time of writing the specification. Even
though the private use language tags are supported they should not be
used to ensure readability across the ecosystem. It is recommended to
follow the conventions for the capitalization of the subtags even though
it is not mandatory as most users are used to that.
]
...
An issuing party might choose to prepend its `$.document.publisher.name`
...
`Example Company security_advisory` in `$.document.category` are
therefore valid categories for the profile "CSAF Base". This is
important to prepare forward compatibility as later versions of CSAF
might add new profiles. Therefore, the values which can be used for the
...
#link(<mixed-integer-and-semantic-versioning>)[6.1.30]), implementations
SHALL map items with integer versioning to semantic versioning by
appending `.0.0` to the corresponding values. The aforementioned rule is
solely used during the processing to create consistent test results.

#quote(block: true)[
This processing rule is not a suggested quick fix for test
#link(<mixed-integer-and-semantic-versioning>)[6.1.30] but
implementation guidance for this edge case.
]
...
#quote(block: true)[
The first item when sorted by `date` has a higher version number than
the second.
]
...
#quote(block: true)[
Dates are used as primary sorting criteria as they correspond to
publications. The assigned numbers might be incorrect. Issuing parties
are advised to investigate the issue and fix errors by providing the
factual revision history with one entry per release. Note that this
might lead to a situation where an invalid revision history is corrected
by modifying the `number` in a revision history item to a lower value as
required by this test. Nevertheless, the newly added item created to
release the corrected version of the document to the intended target
group will have a newer timestamp and therefore show up correctly at
CSAF consumers.
]
...
#link(<product-version-range-rules>)[6.1.50]. Unknown VERS types are
detected via test #link(<unknown-vers-type>)[6.2.51].
...
#quote(block: true)[
Different implementations might support different VERS types. Usually,
unknown VERS types hinder the automated evaluation of VERS. However, it
is expected that the test is able to recognize all officially registered
VERS types. The differentiation will help users analyzing the result of
the test and addressing the issue appropriately.
]
...
#emph[Example 1 (which fails the test):]#box()<unknown-vers-type-eg-1>
...
#quote(block: true)[
The VERS type `someweirdunknownverstype` is a not an officially
registered one. Note: An implementation would also have to state whether
it supports this VERS type.
]
...
#emph[Example 1 (which fails the
test):]#box()<extension-category-essential-eg-1>
...
#quote(block: true)[
The extension category is `essential`. A reader that does not understand
this extension might miss information that is essential to understand
the content of the CSAF document.
]
...
- type
  `$['$defs'].full_product_name_t..product_identification_helper..hashes`:
  The CSAF 2.0 to CSAF 2.1 Converter SHALL keep the order of the items
  during the conversion. If a value occurs multiple times, the CSAF 2.0
  to CSAF 2.1 Converter SHALL convert only the first one and therefore
  making sure that all items are unique.

- type
  `$['$defs'].full_product_name_t..product_identification_helper..hashes[*]..file_hashes`:
  The CSAF 2.0 to CSAF 2.1 Converter SHALL keep the order of the items
  during the conversion. If a value occurs multiple times, the CSAF 2.0
  to CSAF 2.1 Converter SHALL convert only the first one and therefore
  making sure that all items are unique.
...
- type
  `$['$defs'].full_product_name_t..product_identification_helper..sbom_urls`:
  The CSAF 2.0 to CSAF 2.1 Converter SHALL keep the order of the items
  during the conversion. If a value occurs multiple times, the CSAF 2.0
  to CSAF 2.1 Converter SHALL convert only the first one and therefore
  making sure that all items are unique.
...
- type
  `$['$defs'].full_product_name_t..product_identification_helper..x_generic_uris`:
  The CSAF 2.0 to CSAF 2.1 Converter SHALL keep the order of the items
  during the conversion. If a value occurs multiple times, the CSAF 2.0
  to CSAF 2.1 Converter SHALL convert only the first one and therefore
  making sure that all items are unique.
