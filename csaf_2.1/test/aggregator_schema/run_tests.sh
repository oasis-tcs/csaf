...
ORIG_SCHEMA=csaf_2.1/json_schema/aggregator.json
...
validate() {
  ...
  if python3 ${VALIDATOR} ${SCHEMA} $1 ${CSAF_STRICT_SCHEMA} ${CVSS_20_STRICT_SCHEMA} ${CVSS_30_STRICT_SCHEMA} ${CVSS_31_STRICT_SCHEMA} ${CVSS_40_STRICT_SCHEMA} ${CVSS_META_SCHEMA} ${META_SCHEMA} ${SSVC_200_SL_SCHEMA} ${PROVIDER_STRICT_SCHEMA}; then
    printf "%s\n" SUCCESS
  else
    printf "%s\n" FAILED
    FAIL=1
